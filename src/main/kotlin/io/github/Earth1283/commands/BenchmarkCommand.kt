...
class BenchmarkCommand(private val plugin: HardwareAudit) : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        when (args[0].lowercase()) {
            "specs" -> {
                ...
                sender.sendMessage(mm.deserialize("<gray>OS:</gray> <white>${SystemInfo().operatingSystem}</white>"))
            ...
            "claims" -> {
                ...
                hostAuditor.audit { progress -> 
                    sender.sendMessage(mm.deserialize(progress))
                }.thenAccept { results ->
    ...
    private fun displayNeighborResults(sender: CommandSender, result: NetworkScanResult) {
        ...
        if (result.isLocal) {
            sender.sendMessage(mm.deserialize("<gray>Process Scan:</gray> <white>${result.neighborProcesses.size} instances</white>"))
        }
